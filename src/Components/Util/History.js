            ...
            <table className='rounded-md'>
                <thead>
                    {props.user == "bank" ? <tr>
                        ...
                        {props.handle == "requests" && <th className='border p-4 px-4'>Doctor Document</th>}
                ...
                <tbody>
                    {
                        data.map((e, i) =>
                            props.user == "bank" ? <tr className={status == "All" ? "" : status != e.status ? "hidden" : ""}>
                                ...
                                {props.handle == "requests" && <td className='border p-3'>
                                    {e.doctorDocument ? (
                                        <a
                                            href={e.doctorDocument}
                                            target="_blank"
                                            rel="noreferrer"
                                            download={e.documentName || "doctor-document"}
                                            className="text-indianred underline decoration-dotted underline-offset-4 hover:text-red-600"
                                        >
                                            View Document
                                        </a>
                                    ) : "---"}
                                ...
                                <td className='border p-3'><span className={`pill-chip border-2 px-4 py-2 rounded-xl hover:shadow-md ${e.status === "Denied" ? "tag-danger" : e.status === "Pending" ? "tag-neutral" : "tag-success"}`}>{e.status}</span></td>
                            </tr>
                        )
                    }
                </tbody>
